...
class PyGamePlayer(object):
    def __init__(self, force_game_fps=10, run_real_time=False, pass_quit_event=True):
        """
        Clase de resumen para agentes de aprendizaje, como correr refuerzo aprendiendo redes neuronales contra juegos
        de PyGame.

        The get_keys_pressed and get_feedback methods must be overriden by a subclass to use
        Las teclas get presionadas y obtener métodos de retroalimentación deben ser anuladas por una subclase para usar
    ...
    def start(self):
        ...
        # Esto actualizará el contenido de la pantalla completa.
        pygame.display.flip = function_intercept(pygame.display.flip, self._on_screen_update)
        # Actualizar porciones de la pantalla para pantallas de software
