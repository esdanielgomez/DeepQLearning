...
class DeepQPongPlayer(PongPlayer):
    ...
    def __init__(self, checkpoint_path="deep_q_pong_networks", playback_mode=False, verbose_logging=False):
        ...
        :param checkpoint_path: directory to store checkpoints in -  directorio para almacenar puntos de control en
        :type checkpoint_path: str
        :param playback_mode: if true games runs in real time mode and demos itself running -   si los juegos verdaderos se ejecutan en modo de tiempo real y las demos se ejecutan
        :type playback_mode: bool
        :param verbose_logging: If true then extra log information is printed to std out    -   Si es verdadero, la información de registro adicional se imprime para que quede fuera
        :type verbose_logging: bool
        """
        self._playback_mode = playback_mode
        super(DeepQPongPlayer, self).__init__(force_game_fps=8, run_real_time=playback_mode)
        self.verbose_logging = verbose_logging  #SI ES VERDADERO LA INFORMACION DE REGISTRO ADICIONAL SE IMPRIME
        self._checkpoint_path = checkpoint_path #SI ES VERDADERO SE GUARDA EL PUNTO DE CONTROL
        self._session = tf.Session()    #CREAMOS LA RED NEURONAL
        self._input_layer, self._output_layer = DeepQPongPlayer._create_network()#NOS PROPORCIONARA LA CAPA DE ENTRADA Y LA CAPA DE SALIDA
        self._action = tf.placeholder("float", [None, self.ACTIONS_COUNT]) #GUARDARA EL MARCADOR DE POSICION - Placeholder Tensor("Placeholder_1:0", shape=(?, 3), dtype=float32)
        self._target = tf.placeholder("float", [None]) #OBJETIVO -  Tensor("Placeholder_2:0", shape=(?,), dtype=float32)

        """
        Un Tensor es un identificador simbólico de una de las salidas de una Operación. 
        No contiene los valores de la salida de esa operación, sino que proporciona un medio para 
        calcular esos valores en un TensorFlow tf.Session.nsor.
        
        """
        #multiply - multiplica tensores
        readout_action = tf.reduce_sum(tf.multiply(self._output_layer, self._action), reduction_indices=1) #Calcula la suma de elementos a través de las dimensiones de un tensor.
            #redout_action - lee la accion
        cost = tf.reduce_mean(tf.square(self._target - readout_action)) #COSTO
        """
        ...
        """
            train soporta diferentes modelos de entrenamiento
                Optimizer that implements the Adadelta algorithm.
        """
        self._observations = deque()
        self._last_scores = deque()

        # establecer la primera acción para no hacer nada
        self._last_action = np.zeros(self.ACTIONS_COUNT)
        self._last_action[1] = 1

        self._last_state = None #Ultimo estado
        self._probability_of_random_action = self.INITIAL_RANDOM_ACTION_PROB    #probabilidad de accion aleatoria
        self._time = 0
        ...
        self._saver = tf.train.Saver() #Guardar y restaurar los datos
